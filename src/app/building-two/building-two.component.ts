...
@Component({
  selector: 'app-building-two',
  templateUrl: './building-two.component.html',
  styleUrls: ['./building-two.component.css']
})
export class BuildingTwoComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.topicTempSubscription = this.rxStompService.watch('/topic/B2_Temperature').subscribe((Tmessage: Message) => {
      this.receivedTemp.push(Tmessage.body);
      this.temperature = Tmessage.body;
      this.onChangeTemp()
    });
    this.topicLightSubscription = this.rxStompService.watch('/topic/B2_Brightness').subscribe((Lmessage: Message) => {
  ...
  onTurnOnHeat() {
    const message = `UP`;
    this.rxStompService.publish({ destination: '/topic/B2_Temp_Actuator', body: message });
  }
  ...
  onTurnOnLights() {
    const message = `UP`;
    this.rxStompService.publish({ destination: '/topic/B2_Light_Actuator', body: message });
  }
